...
const Dashboard = () => {
  ...
  useEffect(() => {
    // Cargar impresoras conectadas
    const loadPrinters = async () => {
      try {
        const data = await fetchConnectedPrinters();
        setPrinters(data);
        
        // Suscribir a actualizaciones de estado para cada impresora
        data.forEach(printer => {
          const topic = `printer/${printer.id}/status`;
          mqttService.subscribe(topic, handleMqttUpdate);
        });
      } catch (err) {
        console.error('Error loading printers:', err);
        setError('No se pudieron cargar las impresoras. Por favor, inténtalo de nuevo.');
      } finally {
        setLoading(false);
      }
    };
    ...
    // Limpiar suscripciones MQTT al desmontar
    return () => {
      printers.forEach(printer => {
        const topic = `printer/${printer.id}/status`;
        mqttService.unsubscribe(topic, handleMqttUpdate);
      });
    };
  }, []);

  // Manejar actualizaciones MQTT
  const handleMqttUpdate = (message, topic) => {
    // Extraer ID de impresora del tópico (formato: printer/{id}/status)
    const printerId = topic.split('/')[1];
    
    // Actualizar el estado de la impresora específica
    setPrinters(prevPrinters => prevPrinters.map(printer => {
